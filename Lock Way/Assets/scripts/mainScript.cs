...
public class mainScript : MonoBehaviour
{
    ...
    private bool IsLosePosition()
    {
        ...
        {
            if ((door.GetComponent<door2>().codeZoneA == m_state || door.GetComponent<door2>().codeZoneB == m_state) && door.GetComponent<door2>().countOfLives > 0)
            {
                return false;
            }
        }
        ...
        foreach (GameObject status in statusOfDoors)
        {
            ...
            if (status.GetComponent<SpriteRenderer>().color == new Color(100, 0, 100))
                return false;
        }
